...
*参考资料：[Browser-Native LLM Inference](https://tianpan.co/zh/blog/2026-04-17-browser-native-llm-inference-webgpu)（2026年4月）、[WebGPU 官方规范](https://webgpu.org/)、[WebLLM 项目](https://github.com/mlc-ai/web-llm)*`,
  },
  {
    ...
    title: "浏览器是如何\"特殊照顾\"大网站的——WebKit Quirks 深度解析",
    date: "2026-05-16",
    tags: ["\u6d4f\u89c8\u5668", "Web\u6807\u51c6", "WebKit", "\u524d\u7aef", "\u67b6\u6784"],
    excerpt: `当你打开 Safari 访问 Facebook 看视频时，你有没有想过：为什么视频不会像预期那样暂停？为什么播放器行为和 Chrome 不太一样？`,
    content: `## 前言
...
\`\`\`cpp
// Facebook, X (twitter), and Reddit will naively pause a <video> element 
...
这个修复shipping到了**每一台 iOS 设备**上。所有 Safari 用户都受益于这个没有人知道的修复。只有看源代码你才能发现自己的网站被"特殊对待"了。
...
- *Katryna Blaettner: "[Removing a WebKit quirk for FlightAware](https://www.otsukare.info/2023/01/16/webkit-quirks)")*`,
  },
  {
    ...
    title: "Bun 六天 Rust 迁移深度解析：96 万行代码、13000 个 unsafe，以及 AI 重写软件的边界",
    date: "2026-05-16",
    tags: ["Bun", "Rust", "Zig", "AI\u7f16\u7a0b", "\u8fd0\u884c\u65f6", "\u5185\u5b58\u5b89\u5168"],
    excerpt: `2026 年 5 月 11 日，Bun 创始人 Jarred Sumner 在 X 上发了一条推文：`,
    content: `# Bun 六天 Rust 迁移深度解析：96 万行代码、13000 个 unsafe，以及 AI 重写软件的边界
...
- DeepSeek-Knowledge-Distillation: https://github.com/deepseek-ai/DeepSeek-Knowledge-Distillation`,
  ...
  {
    slug: "2026-05-16-llm-test-time-compute-scaling",
    title: "LLM 推理即训练：Test-Time Compute Scaling 的架构革命",
    date: "2026-05-16",
    tags: ["AI", "LLM", "\u67b6\u6784", "\u63a8\u7406\u4f18\u5316"],
    excerpt: `在 LLM 领域，有一个正在发生的范式转移，它的颠覆性不亚于 2020 年的 Scaling Law，但讨论度远没有那么大——**Test-Time Compute Scaling**（推理时计算扩展）。`,
    content: `在 LLM 领域，有一个正在发生的范式转移，它的颠覆性不亚于 2020 年的 Scaling Law，但讨论度远没有那么大——**Test-Time Compute Scaling**（推理时计算扩展）。
...
自回归生成是顺序的，但 **Speculative Decoding** 和 **Beam Search 变体**让多个候选路径可以高效并行生成，代价是略微增加内存使用，但延迟保持在可接受范围内。
...
这意味着推理基础设施不再是"把模型跑起来"那么简单，而是一个需要调度、搜索和评估的**运行时系统**。
...
这实际上是把推理问题建模成了一个**受限的决策过程**，而在传统范式下，推理只是前馈计算。
...
2026 年的 LLM 竞争，已经不只是训练场的军备竞赛——**推理时计算**正在成为新的主战场。`,
  },
  {
...
与此同时，React Foundation 的成立是 React 历史上最重要的治理事件。Linux Foundation 的背书让 React 真正成为社区公共资产，而非单一公司的战略筹码。对于已经在 React 生态深耕的团队，这两件事的组合意味着：React 的稳定性会更强，社区影响力会更深，产品开发可以更专注于业务逻辑而非框架迁移焦虑。`,
  },
  {
    ...
    title: "Rust 图像处理性能提升 6 倍：fast_blur 优化的底层秘密",
    date: "2026-05-16",
    tags: ["Rust", "\u6027\u80fd\u4f18\u5316", "\u56fe\u50cf\u5904\u7406", "\u7b97\u6cd5"],
    excerpt: `最近在看 Rust \`image\` crate 的代码时，发现了一个被合并的 PR（[#2846](https://github.com/image-rs/image/pull/2846)），将 \`fast_blur\` 函数的性能提升了 **5.9 倍**——从 52ms 降到了 8ms。背后的优化思路非常经典，值得拆解。`,
    content: `最近在看 Rust \`image\` crate 的代码时，发现了一个被合并的 PR（[#2846](https://github.com/image-rs/image/pull/2846)），将 \`fast_blur\` 函数的性能提升了 **5.9 倍**——从 52ms 降到了 8ms。背后的优化思路非常经典，值得拆解。
...
*本文涉及的技术指标和数据均来自腾讯云数据库团队公开披露的资料，截至 2026 年 5 月。*`,
  ...
  {
    slug: "2026-05-16-wasi-02-component-model-2026",
    title: "WASI 0.2：WebAssembly 组件模型如何重塑服务端运行时格局",
    date: "2026-05-16",
    tags: ["WebAssembly", "WASI", "Rust", "Wasm", "Serverless"],
    excerpt: `2026 年，WebAssembly 悄悄完成了一次看似"小版本号"、实则革命性的跨越。WASI（WebAssembly System Interface）0.2 正式落地，引入了 Component Model ——一种让 Wasm 模块真正具备跨语言互操作能力的模型。过去我们把 Wasm 当作浏览器里的沙盒执行环境，如今它正在成为服务端基础设施的主流选项`,
    content: `2026 年，WebAssembly 悄悄完成了一次看似"小版本号"、实则革命性的跨越。WASI（WebAssembly System Interface）0.2 正式落地，引入了 Component Model ——一种让 Wasm 模块真正具备跨语言互操作能力的模型。过去我们把 Wasm 当作浏览器里的沙盒执行环境，如今它正在成为服务端基础设施的主流选项。
...
旧版 WASI 的接口（如 \`wasi_filesystem\`）被重新设计为 WIT 格式的组件接口。原来以原始指针传递的 \`buffer\` 类型，现在变成了 WIT 原生的 \`list<u8>\` 或 \`string\`，运行时自动完成内存转换。
